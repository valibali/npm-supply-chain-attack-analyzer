            ...
            if [ -f "$package_file" ]; then
                for package in "${!COMPROMISED_PACKAGES[@]}"; do
                    if grep -q "\"$package\"" "$package_file" 2>/dev/null; then
                        ...
                        # Try to extract version from the file - look for the specific package and its version in dependencies sections only
                        local found_version="unknown"
                        
                        # Check in dependencies, devDependencies, and peerDependencies sections
                        if grep -A 1000 '"dependencies"[[:space:]]*:' "$package_file" 2>/dev/null | grep -B 1000 '^[[:space:]]*}' | grep -q "\"$package\"[[:space:]]*:"; then
                            found_version=$(grep -A 1000 '"dependencies"[[:space:]]*:' "$package_file" 2>/dev/null | grep -B 1000 '^[[:space:]]*}' | grep "\"$package\"[[:space:]]*:" | sed 's/.*"'"$package"'"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/' 2>/dev/null || echo "unknown")
                        elif grep -A 1000 '"devDependencies"[[:space:]]*:' "$package_file" 2>/dev/null | grep -B 1000 '^[[:space:]]*}' | grep -q "\"$package\"[[:space:]]*:"; then
                            found_version=$(grep -A 1000 '"devDependencies"[[:space:]]*:' "$package_file" 2>/dev/null | grep -B 1000 '^[[:space:]]*}' | grep "\"$package\"[[:space:]]*:" | sed 's/.*"'"$package"'"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/' 2>/dev/null || echo "unknown")
                        elif grep -A 1000 '"peerDependencies"[[:space:]]*:' "$package_file" 2>/dev/null | grep -B 1000 '^[[:space:]]*}' | grep -q "\"$package\"[[:space:]]*:"; then
                            found_version=$(grep -A 1000 '"peerDependencies"[[:space:]]*:' "$package_file" 2>/dev/null | grep -B 1000 '^[[:space:]]*}' | grep "\"$package\"[[:space:]]*:" | sed 's/.*"'"$package"'"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/' 2>/dev/null || echo "unknown")
                        fi
                        ...
                        if [ "$found_version" != "unknown" ] && is_version_vulnerable "$found_version" "${COMPROMISED_PACKAGES[$package]}"; then
                            ...
                            record_compromised_package "$package" "VSCode extension $ext_name ($package_file)" "$found_version" "declared"
                        ...
                        for package in "${!COMPROMISED_PACKAGES[@]}"; do
                            if [[ "$nm_package_name" == "$package" ]] || grep -q "\"name\"[[:space:]]*:[[:space:]]*\"$package\"" "$nm_package_file" 2>/dev/null; then
                                ...
                                if [ "$dep_version" != "unknown" ] && is_version_vulnerable "$dep_version" "${COMPROMISED_PACKAGES[$package]}"; then
                                    ...
                                    # Record the finding for the report
                                    record_compromised_package "$package" "VSCode extension $ext_name dependency ($nm_package_file)" "$dep_version" "installed"
...
analyze_package_directories() {
    ...
    for package_dir in "${package_dirs[@]}"; do
        ...
        for package in "${!COMPROMISED_PACKAGES[@]}"; do
            if grep -q "\"$package\"" "$package_file" 2>/dev/null; then
                # Try to extract version from dependencies sections only
                local declared_version="unknown"
                
                # Check in dependencies, devDependencies, and peerDependencies sections
                if grep -A 1000 '"dependencies"[[:space:]]*:' "$package_file" 2>/dev/null | grep -B 1000 '^[[:space:]]*}' | grep -q "\"$package\"[[:space:]]*:"; then
                    declared_version=$(grep -A 1000 '"dependencies"[[:space:]]*:' "$package_file" 2>/dev/null | grep -B 1000 '^[[:space:]]*}' | grep "\"$package\"[[:space:]]*:" | sed 's/.*"'"$package"'"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/' 2>/dev/null || echo "unknown")
                elif grep -A 1000 '"devDependencies"[[:space:]]*:' "$package_file" 2>/dev/null | grep -B 1000 '^[[:space:]]*}' | grep -q "\"$package\"[[:space:]]*:"; then
                    declared_version=$(grep -A 1000 '"devDependencies"[[:space:]]*:' "$package_file" 2>/dev/null | grep -B 1000 '^[[:space:]]*}' | grep "\"$package\"[[:space:]]*:" | sed 's/.*"'"$package"'"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/' 2>/dev/null || echo "unknown")
                elif grep -A 1000 '"peerDependencies"[[:space:]]*:' "$package_file" 2>/dev/null | grep -B 1000 '^[[:space:]]*}' | grep -q "\"$package\"[[:space:]]*:"; then
                    declared_version=$(grep -A 1000 '"peerDependencies"[[:space:]]*:' "$package_file" 2>/dev/null | grep -B 1000 '^[[:space:]]*}' | grep "\"$package\"[[:space:]]*:" | sed 's/.*"'"$package"'"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/' 2>/dev/null || echo "unknown")
                fi
                ...
                if [ "$declared_version" != "unknown" ] && is_version_vulnerable "$declared_version" "${COMPROMISED_PACKAGES[$package]}"; then
                    ...
                    # Record the finding
                    record_compromised_package "$package" "$package_file (dependency)" "$declared_version" "declared"
                ...
                for vulnerable_package in "${!COMPROMISED_PACKAGES[@]}"; do
                    if [[ "$installed_package_name" == "$vulnerable_package" ]]; then
                        ...
                        if [ "$installed_version" != "unknown" ] && is_version_vulnerable "$installed_version" "${COMPROMISED_PACKAGES[$vulnerable_package]}"; then
                            ...
                            record_compromised_package "$installed_package_name" "$installed_package_dir (installed: $installed_version)" "$installed_version" "installed"
                ...
                for package in "${!COMPROMISED_PACKAGES[@]}"; do
                    if grep -q "\"$package\"" "$lock_file" 2>/dev/null; then
                        ...
                        if [ "$locked_version" != "unknown" ] && is_version_vulnerable "$locked_version" "${COMPROMISED_PACKAGES[$package]}"; then
                            ...
                            # Record the finding
                            record_compromised_package "$package" "$lock_file (locked: $locked_version)" "$locked_version" "installed"
...
declare -g -A DECLARED_ONLY_PACKAGES  # Packages only in package.json (not installed)
declare -g -A INSTALLED_PACKAGES      # Actually installed packages
            ...
            if [ -n "$file_hash" ]; then
                for malicious_hash in "${MALICIOUS_HASHES[@]}"; do
                    if [[ "$file_hash" == "$malicious_hash" ]]; then
                        ...
                        record_compromised_package "MALICIOUS_FILE" "$file (hash: $file_hash)" "CRITICAL" "installed"
        ...
        for file in "${text_files[@]}"; do
            if [ -f "$file" ] && [ -r "$file" ]; then
                ...
                if [ -n "$matches" ]; then
                    ...
                    record_compromised_package "SUSPICIOUS_PATTERN" "$file (pattern: $pattern)" "SUSPICIOUS" "declared"
...
record_compromised_package() {
    ...
    local is_installed="$4"  # "installed" or "declared"
    ...
    if [ "$is_installed" = "installed" ]; then
        INSTALLED_PACKAGES["$package_name"]="$found_version"
    else
        DECLARED_ONLY_PACKAGES["$package_name"]="$found_version"
    fi
...
generate_compromised_packages_report() {
    ...
    # Separate installed vs declared packages
    local installed_packages=()
    local declared_only_packages=()
    
    for package in "${!INSTALLED_PACKAGES[@]}"; do
        installed_packages+=("$package")
    done
    
    for package in "${!DECLARED_ONLY_PACKAGES[@]}"; do
        declared_only_packages+=("$package")
    done
...
=============================
🚨 ACTUALLY INSTALLED (CRITICAL): ${#installed_packages[@]} packages
⚠️  DECLARED ONLY (WARNING): ${#declared_only_packages[@]} packages
...
EOF
        ...
        # Show installed packages first (critical)
        if [ ${#installed_packages[@]} -gt 0 ]; then
            cat >> "$report_file" << EOF
🚨 CRITICAL - ACTUALLY INSTALLED COMPROMISED PACKAGES:
=====================================================

⚠️  IMMEDIATE ACTION REQUIRED! These packages are installed and active:

EOF
            
            IFS=$'\n' sorted_installed=($(sort <<<"${installed_packages[*]}"))
            unset IFS
            
            local counter=1
            for package in "${sorted_installed[@]}"; do
                echo "${counter}. 🚨 PACKAGE: $package (INSTALLED)" >> "$report_file"
                echo "   Compromised versions: ${FOUND_COMPROMISED_PACKAGES[$package]}" >> "$report_file"
                echo "   Installed version: ${INSTALLED_PACKAGES[$package]}" >> "$report_file"
                echo "   🔥 RISK LEVEL: CRITICAL - REMOVE IMMEDIATELY!" >> "$report_file"
        ...
        # Show declared-only packages (warning level)
        if [ ${#declared_only_packages[@]} -gt 0 ]; then
            cat >> "$report_file" << EOF
⚠️  WARNING - DECLARED BUT NOT INSTALLED PACKAGES:
=================================================

These packages are mentioned in package.json but not currently installed:

EOF
            
            IFS=$'\n' sorted_declared=($(sort <<<"${declared_only_packages[*]}"))
            unset IFS
            
            local counter=1
            for package in "${sorted_declared[@]}"; do
                echo "${counter}. ⚠️  PACKAGE: $package (DECLARED ONLY)" >> "$report_file"
                echo "   Compromised versions: ${FOUND_COMPROMISED_PACKAGES[$package]}" >> "$report_file"
                echo "   Declared version: ${DECLARED_ONLY_PACKAGES[$package]}" >> "$report_file"
                echo "   🛡️  RISK LEVEL: WARNING - Not currently installed" >> "$report_file"
                echo "   Found in locations:" >> "$report_file"
...
FOR INSTALLED PACKAGES (CRITICAL PRIORITY):
🚨 1. STOP using the affected packages immediately
🚨 2. Remove compromised package versions from node_modules
🚨 3. Update package.json to exclude vulnerable versions
🚨 4. Run 'npm install' or 'yarn install' to reinstall clean versions
🚨 5. Run security scans after cleanup

FOR DECLARED-ONLY PACKAGES (MEDIUM PRIORITY):
⚠️  1. Update package.json to exclude vulnerable versions
⚠️  2. Ensure lock files don't reference vulnerable versions
⚠️  3. Be cautious when running 'npm install' - vulnerable versions may be installed

GENERAL ACTIONS:
🔍 1. Review all locations where compromised packages are found
🔄 2. Update to clean, verified versions
🛡️  3. Run security scans after cleanup
...
INSTALLED PACKAGES:
- RISK LEVEL: CRITICAL
- IMPACT: Active malicious code execution possible
- URGENCY: Immediate action required

DECLARED-ONLY PACKAGES:
- RISK LEVEL: WARNING
- IMPACT: Potential future installation of malicious code
- URGENCY: Update before next installation

EOF
    fi
    
    # Generate CSV report for easier processing
    echo "Package Name,Compromised Versions,Installation Status,Locations,Risk Level" > "$csv_report_file"
    ...
    if [ ${#unique_packages[@]} -gt 0 ]; then
        ...
        for package in "${installed_packages[@]}"; do
            ...
            echo "\"$package\",\"${FOUND_COMPROMISED_PACKAGES[$package]}\",\"INSTALLED\",\"$locations_csv\",\"CRITICAL\"" >> "$csv_report_file"
        done
        
        # Add declared-only packages
        for package in "${declared_only_packages[@]}"; do
            local locations_csv
            locations_csv=$(echo "${FOUND_PACKAGE_LOCATIONS[$package]}" | tr '|' ';')
            echo "\"$package\",\"${FOUND_COMPROMISED_PACKAGES[$package]}\",\"DECLARED_ONLY\",\"$locations_csv\",\"WARNING\"" >> "$csv_report_file"
        done
    fi
    ...
    if [ ${#unique_packages[@]} -gt 0 ]; then
        echo
        if [ ${#installed_packages[@]} -gt 0 ]; then
            echo -e "${RED}🚨 CRITICAL - INSTALLED COMPROMISED PACKAGES:${NC}"
            echo -e "${RED}=============================================${NC}"
            for package in "${installed_packages[@]}"; do
                echo -e "${RED}• $package${NC} (${FOUND_COMPROMISED_PACKAGES[$package]}) - ${RED}REMOVE IMMEDIATELY!${NC}"
            done
            echo
        fi
        
        if [ ${#declared_only_packages[@]} -gt 0 ]; then
            echo -e "${YELLOW}⚠️  WARNING - DECLARED BUT NOT INSTALLED:${NC}"
            echo -e "${YELLOW}=========================================${NC}"
            for package in "${declared_only_packages[@]}"; do
                echo -e "${YELLOW}• $package${NC} (${FOUND_COMPROMISED_PACKAGES[$package]}) - ${YELLOW}Update package.json${NC}"
            done
            echo
        fi
    fi
